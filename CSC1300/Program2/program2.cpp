...
void RollDie(int dieRoll){      // Function used to print die art
	switch(dieRoll){
		case 1:
			cout << " _________\n";
			cout << "|         |\n";
...
void GoodFace(){    // Function used to print happy face art
...
void BadFace(){     // Function used to print sad face art
...
int main(){
	...
	do {    // Main game loop. Only loops if user chooses to play again

		    cout << endl << "How many players? (1-" << MAX_PLAYERS << "):" << endl;

        do{
            while(!(cin >> numPlayers)){ // This while loop doesn't allow characters to be entered, thus preventing an infinite loop
                cout << endl << "Invalid input. Please enter a number (1-" << MAX_PLAYERS << "):" << endl;
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                }
		    if(numPlayers < 1 || numPlayers > MAX_PLAYERS){
			    cout << endl << "Invalid number of players. Please enter (1-" << MAX_PLAYERS << "):" << endl;
                cin.clear();
			    cin >> numPlayers;
			    cin.ignore();
                }
		...
		for(i = 0; i < numPlayers; ++i){
			cout << "\nPlayer " << i+1 << ", What is your name?" << endl << "Name: ";
			getline(cin, names[i]);
		...
		for(i = 0; boardSpace[i] < NUM_SPACES; ++i){
			...
			else{   // No need to check if goodOrBad == 2 since there are only 2 possible outcomes: 1 or 2
				inFS.open("bad.txt");
				if(!inFS.is_open()){
