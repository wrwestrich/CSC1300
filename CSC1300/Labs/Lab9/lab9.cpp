...
int main() {
        ...
        cout << endl << "Maintain confidentiality at expense of availability (no GPA):" << endl;
        for(i = 0; i < NUM_STUDENTS; ++i)
            cout << setfill(' ') << setw(15) << left << student[i].id << student[i].name << endl;

        cout << endl << "Maintain confidentiality and availability at expense of integrity (noisy GPA):" << endl;
        for(i = 0; i < NUM_STUDENTS; ++i){
