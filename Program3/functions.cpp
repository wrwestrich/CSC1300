...
string WordWrap(const char *text, int consoleSize)
{ // Function using console window size to wrap text
...
void ValidNum(float &input)
{ // Checks for a valid input when adding a creature
  do
  {
    while (!(cin >> input))
    {
...
int EnterCreatures(int numCreatures, Creatures creatures[])
{
  ...
  case 1:
    ...
    while (69)
    { // Loop relies on a break at the end of the file to terminate
      if (creatures[i].name != "")
      {
        ++i;
        continue;
      ...
      j = 0;
      k = 0;
      while (j < numCreatures)
      { // Loop skips any "deleted" creature by passing by a blank name
        if (creatures[k].name == "")
        {
          ++k;
          continue;
        }
        if (creatures[k].name == tempString)
        { // Checks if creature already exists
          exists = true;
          break;
        }
        ++j;
      ...
      if (exists)
      { // Skips an already existing creature by reading in all corresponding data to junk variable
        exists = false;
        skipped = true;
        for (j = 0; j < 9; ++j)
          getline(inFile, tempString, '#');
    ...
    if (maxReached || skipped)
      cout << endl
           << "Not all creatures in file were read because the max number\nof creatures was reached or a creature already existed." << endl;

    cout << endl
         << creaturesAdded << " creatures successfully loaded from " << fileName << endl
         << endl;
    break;
  ...
  case 2:
    ...
    ++numCreatures;
    cout << endl
         << "\nThe " << creatures[i].name << " has been added." << endl
         << endl;
    break;
...
int DeleteCreature(int numCreatures, Creatures creatures[])
{
  ...
  while (i < numCreatures)
  {
    ...
    if (creatureName == creatures[j].name)
    {
      ...
      --numCreatures;
      cout << endl
           << creatureName << " has been removed." << endl
           << endl;
    }
    ++i;
    ++j;
  ...
  if (!creatureRemoved)
  { // Displays if a creature was not found
    cout << endl
         << creatureName << " was not found." << endl
         << endl;
...
void PrintCreatures(int numCreatures, Creatures creatures[])
{
  ...
  if (numCreatures == 0)
  {
    cout << "\n\nThere are no creatures to print out. Please add creatures to continue." << endl
         << endl;
    return;
  }
  ...
  while (i < numCreatures)
  {
    ...
    cout << setw(consoleSize) << left << WordWrap(creatures[j].description.c_str(), consoleSize) << endl
         << endl;
    ...
    cout << setw(20) << left << "Dangerous?" << dangerous << endl
         << endl;
    ...
    cout << setw(50) << left << "Cost per hour of taking care of the creature:"
         << "$ " << creatures[j].costData.costPerHour << endl;
    cout << setw(50) << left << "Cost to feed creature:"
         << "$ " << creatures[j].costData.costFood << endl;
    cout << setw(50) << left << "Grooming and supplies cost:"
         << "$ " << creatures[j].costData.costMat << endl
         << endl;
...
void PrintStatistics(int numCreatures, Creatures creatures[])
{
  ...
  while (i < numCreatures)
  { // Adds the cost of each creature together and also adds to total cost
    if (creatures[j].name == "")
    {
      ++j;
      continue;
    }
    totalCostEach[j] = creatures[j].costData.hours * creatures[j].costData.costPerHour + creatures[j].costData.costFood + creatures[j].costData.costMat;
  ...
  cout << endl
       ...
       << "$";
  cout << setw(15) << right << fixed << setprecision(2) << total << endl
       << endl;
}
  ...
  do
  {
    ...
    else if (save == 'y' || save == 'Y')
    {
      ...
      do
      {
        ...
        {
          cout << endl
               << fileName << " could not be opened. Please try again." << endl;
          fileOpened = false;
        }
        else
          fileOpened = true;
